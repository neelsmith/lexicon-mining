...
md"""# Extract morphological data from Lewis-Short's *Latin Dictionary*"""
...
# ╔═╡ 73f6b266-64d6-4898-bb8b-747ee6fcdec0
lewisshort[617]
...
function analyzels(u; ls = lewisshort)
	...
	if isempty(article) || isnothing(article)
		...
		display = [
		"## Analysis by ChatGPT",
		string("`", extractmorph(article.entry), "`")
		]
...
@isdefined(vocab) && !isempty(vocab) ? Markdown.parse(analyzels(vocab)) : nothing
