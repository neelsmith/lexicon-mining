...
The information in these named can be quite useful by itself. Let's look at a couple of examples.
...
We can write a function to format the entry like a familiar glossary or vocabulary entry in a textbook. We'll compose a single string with some highlighting in Markdown to make the entry more easily readable.
...
We these our new function map each article to a single string for one glossary entry, then join the entries together, separating them with two newlines (`\n\n`).
```{julia}
glosses = map(tpl -> markdown_gloss(tpl), sliver)
glosstext = "#### Sample glossary\n\n" * join(glosses, "\n\n")
```
The result is a single string with our Markdown glossary. We can use `Markdown.parse` to get a visual rendering of the string.
...
The Julia `StatsBase` module will count occurrences of a value for us; if we convert the results to an ordered dictionary, we can sort the results.
